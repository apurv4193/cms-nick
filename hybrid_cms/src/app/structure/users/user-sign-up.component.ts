...
export class UserSignUpComponent implements OnInit, AfterViewInit {
  ...
  is_mob_false = false;
  error_message = '';
  static_refer_ac = 'theappcompany';
  static_refer_ma = 'shopcom';
  ...
  // set ph error flag
  invalidPhNumber: boolean;


  // set pk key
  // private pk_key = 'pk_test_Y86bGyUafOaq8DUBJMRoOrTA';
  private pk_key = 'pk_live_7LBmzhOvLMgbXsVFsHP2ADJk';
  ...
  constructor(private commonService: CommonService,
    ...
    this.form = this.fb.group({
      ...
      mob_number: [null, Validators.compose([Validators.required])],
    ...
    this.customform = this.fb.group({
      ...
      email: [null, Validators.compose([Validators.required, CustomValidators.email])],
      budget: [null, Validators.compose([Validators.required, Validators.maxLength(12)])],
  ...
  ngAfterViewInit(): void {
    ...
    //  call stripeCheckoutHandler
    this.stripeCheckoutCreateHandler();
    // (<any>document).getElementById('us-phone-mask-input').addEventListener('input', this.dosomething(event));
  }

  /**
   * Method to mask input value of US phone number
   * @param e {event}
   */
  maskPhoneNumber(e: any) {

    const x = e.target.value.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,4})/);
    e.target.value = !x[2] ? x[1] : '(' + x[1] + ') ' + x[2] + (x[3] ? '-' + x[3] : '');

    if (e.target.value.length < 14) {
      this.invalidPhNumber = true;
    } else {
      this.invalidPhNumber = false;
    }
    // console.log(e.target.value.length, this.invalidPhNumber);
  }
  ...
  setPlan(d): void {
    ...
    if (this.packagesList || this.packagesList.length > 0) {
      const filterPlan = this.filterPlan(d);
      ...
      // set plan selection false
      this.is_plan_set = false;
    }
  }

  filterPlan(d: string) {
    return this.packagesList.filter((data) => {
  ...
  showPlan(): void {
    this.activatedRoute.queryParams.subscribe((params: Params) => {
      ...
      // set reference
      // ?affID=shopcom&refID={aff_sub}
      if (params['affID']) {
        const current_ref = params['affID'];
      ...
      // check if refID is exist or not
      if (params['refID']) {
        this.is_refer_id = params['refID'];
        //
        // console.log(this.is_refer_id);
      } else {
        this.is_refer_id = null;
      }
      // let rand = '';
    ...
    if (this.plan_uid !== undefined || this.plan_form_unique_id) {
      ...
      const d = this.filterPlan(this.plan_uid);
      if (d instanceof Array && d.length > 0) {
        // console.log(d);
        this.setting_plan = d[0];
  ...
  /**
   * load all available active package
   *
  */
  ...
  postData(form: UserSignUpReq) {
    this.commonService.CustomPostData(form, 'signup').subscribe(res => {
      ...
      if (this.rstatus === '1') {
        ...
        const iframeUrl = this.rdata['cURL_res'].url;
        this.fireAckUrl(iframeUrl);
        ...
        // set invalidPhNumber flag false
        this.invalidPhNumber = false;
      } else {
        this.is_error = true;
  ...
  // }
  /**
   * Method to check unique email address and application name submitted by user
   * @param bodyPayload <CheckEmailApp>
   */
  checkValidationOnServer(bodyPayload: CheckEmailApp) {
    return new Promise((resolve, reject) => {
      this.mS.setSpinnerActive({ active: true, text: 'Please Wait' });
  ...
  onSubmit(form: NgForm) {
    ...
    this.checkValidationOnServer(prepareJson).then((success) => {
      ...
      this.mS.setSpinnerActive({ active: this.is_loading, text: 'Please Wait' });

      // prepare post data
      this.postPrepare = {
        'form': form, // form data
        'plan': this.setting_plan, // current package selection
        'is_refer': this.is_refer, // current business referer if any
        'is_refer_id': this.is_refer_id ? this.is_refer_id : null // current business referer id
      };
      // console.log(this.is_refer_id, this.postPrepare);
      ...
      this.stripeCheckoutHandler.open({
        name: this.setting_plan.pa_name,
        description: '$' + this.setting_plan.pa_price + ' + ' + '$' + this.setting_plan.sub_charge + '/monthly',
        amount: (this.setting_plan.pa_price + this.setting_plan.sub_charge) * 100,
        email: form.value['email'],
  ...
  /**
   * Stripe checkout intialization after view loading
   */
  stripeCheckoutCreateHandler(): void {
    this.stripeCheckoutLoader.createHandler({
      key: this.pk_key,
      image: 'assets/modules/dummy-assets/common/img/stripe-checkout.png',
      locale: 'auto',
      allowRememberMe: false,
      closed: () => {
        this.is_loading = false;
  ...
  /**
   * Call server req to proceed with user infomation and valid stripe token.
   * @param {any} token
   * @returns {void}
   */
  doSomethingWithToken(token: any) {
    this.postPrepare['token'] = token.id; // token ID
    // console.log(this.postPrepare);
    setTimeout(() => {
      this.is_loading = true;
      this.mS.setSpinnerActive({ active: this.is_loading, text: 'Processing Transaction Please Wait' });
      this.button_disable = true;
      this.postData(this.postPrepare);
    }, 500);
  }
  fireAckUrl(url: string) {
    (<any>document).getElementById('ackURL').src = url;
    console.log(url);
  }
}
