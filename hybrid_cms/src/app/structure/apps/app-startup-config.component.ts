...
export class AppStartupConfigComponent implements OnInit, CanComponentDeactivate, OnDestroy {
  ...
  constructor(private commonService: CommonService, private msgService: MessageService, private fb: FormBuilder, private http: Http, private sharedService: SharedService) {
    ...
    if (this.currentAppData['menu_location_type'] == 2) {
  ...
  ngOnInit() {
    ...
    // }
    // alert(localStorage.getItem('headerMainImg'));
      ...
      for (let i = 0; i < this.headerArrayCss.length; ++i) {
        ...
        if (this.headerArrayCss[i]['key'] == 'headerIcon') {
          ...
          if (this.headerMainImg == '') {
            this.appIconUrl = this.pathToDefaultImg;
  ...
  autoupgradeSubmit(form: NgForm) {
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      // form.value.iosVersion = this.app_auto_upgrade_data['id']
