...
class GeneralDiscussionController extends Controller
{
    ...
    public function storeDicussion(Request $req) {
        ...
        $sender_user_data = JWTAuth::parseToken()->authenticate();
        ...
        if($sender_user_data) {
            ...
            if( $is_admin == 1 && isset($getAppCreatorId) && count($getAppCreatorId) > 0)
            {   
                //
            ...
            else
            {
                ...
                $prepareArrMailer['user_name'] = $sender_user_data->first_name.' '.$sender_user_data->last_name;
                $prepareArrMailer['user_email'] = $sender_user_data->email;
            }
            if(isset($receiver_id) && isset($app_id) && isset($text)){
    
                $arr['app_id'] = $app_id;
                $arr['sender_id'] = $sender_user_data->id;
                ...
                {
                    $outArr['status'] = 0;
                    $outArr['message'] = 'Oops Something wrong Happen While Saving Data';
                ...
                else
                {
                    ...
                    $outArr['mail_message'] = trans('appmessages.default_error_msg');
            ...
            {
                $outArr['status'] = 0;
                $outArr['message'] = trans('appmessages.default_error_msg');
    ...
    public function getDicussionByAppID(Request $req)
    {
        ...
        if( isset($appid) && $user )
        {
            $getDicussion = $this->appGeneralDicussObj->getDicussionByAppId($appid, $user);
        ...
        {
            $outArr['status'] = 0;
            $outArr['data'] = trans('appmessages.default_error_msg');
