...
class StripPackagesController extends Controller {
    ...
    public function testSignup(Request $request) {
        ...
        if (!is_array($charge) && $charge->charge && $charge->subscriptions) {
        ...
        if (!$user && $st_status === "succeeded") {
            ...
            if ($userDetail) {
                ...
                if($is_refer_id != 'null') {
                    $insertTrans['is_refer_id'] = $is_refer_id;
                }                
                ...
                // send affiliate link
                $payment_cost =  number_format((float)(($st_amount/100) + ($charge->subscriptions->plan->amount/100)), 2, '.', '');
                $affiliate_url = "http://tracking.shopmarketplacenetwork.com/aff_l?offer_id=5715&amount=" . $payment_cost . "&adv_sub=". $is_refer_id ;
                // $response_cURL_code = $this->cURL($affiliate_url);
                ...
                // $outputArray['cURL_res'] =  ['res_code' => $response_cURL_code, 'url' => $affiliate_url];
                $outputArray['cURL_res'] =  ['url' => $affiliate_url];
    ...
    public function create_stripe_customer($user, $plan_id, $stripe_token, $card_holdername = null) {
        ...
        try {
            ...
            // One time Charge
            $customer->charge = \Stripe\Charge::create(array("amount" => $subscription_plan->pa_price * 100, "currency" => "usd", "description" => "The APP Company Platform Activation for (" . $user->email . ")", "customer" => $customer->id));
            $customer->subscriptions = $customer->subscriptions->create($plan_options);
        }
        catch(\Stripe\Error\Card $e) {
            // throw $e;
            
            return $this->stripe_Error_Parser($e);
        }
    ...
    public function stripe_Error_Parser($e){
        $body = $e->getJsonBody();
        $err  = $body['error'];

        $outputArray['status'] = '0';
        $outputArray['message'] = $err['message'];
    ...
    public function webhookListener() {
        try {
            ...
            else if($event_json->type == Config::get('strip_enum.STRIPE_EVENT_JSON.invoice_payment_succeeded') && sizeof($user) == 1 )
            {
                // set description for successful subscription payment
                $ch = \Stripe\Charge::retrieve($event_json->data->object->charge);
                $ch->description = "The APP Company Platform Monthly Service for (" . $user->email . ")";
                $ch->save();
                // file_put_contents("webhook-".date('Y-m-d-H-i-s').'.txt', print_r($event_json,true));
    ...
    public function checkEmailAppExist(Request $req)
    {
        ...
        if( isset($email) && isset($app) ){
            ...
            if($email_exist != null){
                $outputArray['email'] = trans('appmessages.userexistwithemail');
            } else {
                $outputArray['email'] = null;
            }
            if($app_exist != null) {
                $outputArray['app_name'] = trans('appmessages.app_name_unique');
            } else {
