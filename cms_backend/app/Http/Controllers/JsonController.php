...
{
    public function __construct(AppRepository $AppRepository,AppMenuRepository $AppMenuRepository,EmailTemplateRepository $EmailTemplateRepository)
    {
        ...
        $this->menuTutorialOriginalImageUploadPath = Config::get('constant.menuTutorialOriginalRelativePath');
        $this->menuTutorialThumbImageUploadPath= Config::get('constant.menuTutorialThumbRelativePath');
    ...
    {       
        ...
        if ($user)
        {
            ...
            if(isset($appData->menu_location_type) && !empty($appData->menu_location_type))
            {
                $mainJSON['appInfo']['appType'] = ($appData->menu_location_type == '2') ? 'tabmenu' : 'sidemenu';
            }
            ...
            if (isset($appMenuData) && count($appMenuData) > 0) 
            {
                ...
                foreach($nLevelMenu as  $tutorialKey => $checkTutorialOnStart)
                {
                    if($checkTutorialOnStart && $checkTutorialOnStart['menuTypeId'] == '10' && !empty($checkTutorialOnStart['css']))
                    {
                        foreach($appMenuData->toArray() as  $subTutorialKey => $subCheckTutorialOnStart)
                        {
                            $subCheckTutorialOnStart = (array)$subCheckTutorialOnStart;
                            if($subCheckTutorialOnStart && $subCheckTutorialOnStart['menu_type'] == 10 && !empty($subCheckTutorialOnStart['menu_type_json_data']))
                            {
                                $subMenuTypeDecodeJsonData = json_decode($subCheckTutorialOnStart['menu_type_json_data']);
                       
                                if($subMenuTypeDecodeJsonData && count($subMenuTypeDecodeJsonData) > 0 && isset($subMenuTypeDecodeJsonData->show_tutorial) && !empty($subMenuTypeDecodeJsonData->show_tutorial) && $subMenuTypeDecodeJsonData->show_tutorial == 'true')
                                {
                                    $mainJSON['appInfo']['tutorial']['flag'] = 1;
                                    $mainJSON['appInfo']['tutorial']['index'] = $tutorialKey+1;
                                }
                            }
...
/* ************************************************************************************************************* */    
            ...
            $mainJSON['url'] = array(
                'imageUrl' => "http://theappcompany.com/hybrid/taccms/".$appData->app_code."/"
            );
...
// tabmenu css create
            ...
            if(isset($sidemenuCssArray->sideMenuCss[3]->tabMenu) && !empty($sidemenuCssArray->sideMenuCss[3]->tabMenu)) {
                ...
                for ($i = 0; $i < sizeof($finalMainMenuCss); $i++) 
                {
                    ...
                    else if($finalMainMenuCss[$i]->key == "tabSelectedColor")
                    {
                        if($finalMainMenuCss[$i]->value)
                        {
                            $mainMenuTabArr = array_merge($mainMenuTabArr, array(
                    ...
                    else if($finalMainMenuCss[$i]->key == "tabNonSelectedColor")
                    {
                        if($finalMainMenuCss[$i]->value)
                        {
                            $mainMenuTabArr = array_merge($mainMenuTabArr, array(
                    ...
                    else if($finalMainMenuCss[$i]->key == "tabBackgroundColor")
                    {
                        if($finalMainMenuCss[$i]->value)
                        {
                            $mainMenuTabArr = array_merge($mainMenuTabArr, array(
...
//                        if(isset($finalMainMenuCss[5][0]->menutype)) {
...
//          Now download menu images 
            ...
            if(isset($appMenuData) && !empty($appMenuData) && count($appMenuData) > 0)
            {
                foreach($appMenuData as $key => $menuImage)
                {
                    ...
                    if(isset($menuImage->menu_type_json_data) && !empty($menuImage->menu_type_json_data) && $menuImage->menu_type == 8)
                    {
                        $menuTypeJsonDataDecode = json_decode($menuImage->menu_type_json_data);

                        if(!empty($menuTypeJsonDataDecode) && isset($menuTypeJsonDataDecode->pdfUrl) && $menuTypeJsonDataDecode->pdfName)
                        {
                            $menuPdf = public_path($this->menuPdfUploadPath.$menuTypeJsonDataDecode->pdfName);
                            $allScreenShots[] = $menuPdf;
                            $isDownloadImage = true;
                        }
                    }
                    // for Tutorial Images if have any
                    if(isset($menuImage->menu_type_json_data) && !empty($menuImage->menu_type_json_data) && $menuImage->menu_type == 10)
                    {
                        $menuTypeJsonDataDecode = json_decode($menuImage->menu_type_json_data);
                        
                        for ($i=0; $i < count($menuTypeJsonDataDecode->tutorial_add_more_fields); $i++) { 
                            # code...
                            if($menuTypeJsonDataDecode->tutorial_add_more_fields[$i]->video_url == '') {
                                # yes mediaType is Image
                                // $buildArr = ['mediaType'=> 'image','mediaValue'=> $menuTypeJsonDataDecode->menuTypeTutorialImages[$i]->filePathOriginal,'displayOrder'=>$menuTypeJsonDataDecode->tutorial_add_more_fields[$i]->display_order];
                                // $element['value'][$i]= (object)$buildArr;
                                $menuTutorial = public_path($this->menuTutorialOriginalImageUploadPath.$menuTypeJsonDataDecode->menuTypeTutorialImages[$i]->fileName);
                                $allScreenShots[] = $menuTutorial;
                                $isDownloadImage = true;
            ...
            {            
                ...
                $backupFolder = date('Y-m-d-H:i');
                                
                if(ftp_mkdir($conn_id, $versionDirectoryName."/".$backupFolder))
                {       
                    ftp_put($conn_id, $versionDirectoryName."/".$backupFolder."/setting.json", public_path() . "/json/".$appData->app_code."/".$appData->version."/setting.json", FTP_ASCII);   
...
//                  For Notifications
                    ...
                    elseif($element['menu_type'] == 6)
                    {
                        ...
                        if(isset($menuTypeJsonDataDecode->notification_text_fields) && $menuTypeJsonDataDecode->notification_text_fields != null && !empty($menuTypeJsonDataDecode->notification_text_fields))
                        {
                            $notiTextFieldArray = [];
                            foreach($menuTypeJsonDataDecode->notification_text_fields as $notiKey => $notificationText)
                            {
                                $notiTextFieldArray[$notiKey]['title'] = $notificationText->add_text_field;
                            }                            
                            $element['value'] = $notiTextFieldArray;
                        }
                        else
                        {
                            $element['value'] = '';
...
//              For PDF                    
                if(!empty($menuTypeJsonDataDecode) && $element['menu_type'] == 8)
                {
                    if(isset($menuTypeJsonDataDecode->pdfUrl) && isset($menuTypeJsonDataDecode->pdfName) )
                    {
                        $element['value'] = Config::get('constant.IMAGES_JSON_PATH').$menuTypeJsonDataDecode->pdfName;
                    }
                } 
                ...
                if(isset($menuTypeJsonDataDecode->menuTypeTutorialImages))
                {
                    ...
                    if(!empty($menuTypeJsonDataDecode->menuTypeTutorialImages) 
                    && !empty($menuTypeJsonDataDecode->tutorial_add_more_fields)) {
                        ...
                        for ($i=0; $i < count($menuTypeJsonDataDecode->tutorial_add_more_fields); $i++) { 
                            ...
                            if($menuTypeJsonDataDecode->tutorial_add_more_fields[$i]->video_url == '') {
                                ...
                                $buildArr = ['mediaType'=> 'video','mediaValue'=> $menuTypeJsonDataDecode->tutorial_add_more_fields[$i]->video_url,'displayOrder'=>$menuTypeJsonDataDecode->tutorial_add_more_fields[$i]->display_order];
                                $element['value'][$i]= (object)$buildArr;
                            }
                        }
                ...
                unset($element['menu_name'], $element['is_parent'], $element['app_basic_id'], $element['menu_icon'], $element['order'], $element['menu_level'], $element['menu_type'], $element['menu_type_json_data'], $element['is_display_on_app'], $element['created_at'], $element['updated_at'], $element['status']);    
    ...
    public function uploadOnFTP($appCode,$versionName,$jsonFileName,$imageZipFileName)
    {   
       $ftp_server = Config::get('constant.FTP_SERVER');       
       $ftp_user_name = Config::get('constant.FTP_USERNAME');
       $ftp_user_pass = Config::get('constant.FTP_PASSWORD');
...
//      return response()->json($outputArray);
    ...
    public function publishAppInit(Request $req)
    {
        ...
        {
            $jsonData = $this->generateJsonData($req);
            ...
            // $app_data = DB::table('app_basic')
            //             ->where('id', $appId)
            //             ->select('id', 'app_name','app_code','version','app_unique_id', 'app_publish_status')
            //             ->first();
            ...
            $passArr = (object)['first_name'=>$fName,
            'last_name'=>$lName,
            'email'=>$Email,
        ...
        if($emailTemplateContent)
        {
            ...
            if($dataT)
            {
                try 
                {
                    ...
                    $outputArray['status'] = '0';
                    $outputArray['message'] = trans($ex->getMessage());
                }
